<?php 
    class User{
        ...
        public function checkLogin($email,$password){
            $pass = md5($password);
            $sqlQuery = "SELECT * FROM user WHERE email='$email' AND password='$pass' AND type='admin'";
            $rs = $this->conn;
        ...
        public function addUser($firstname, $lastname, $email, $password, $userType="member", $cv=""){
            $pass = md5($password);
            $sqlQuery = "INSERT INTO user (firstname,lastname,email,password,type,cv) VALUES ('$firstname','$lastname','$email','$pass','$userType','$cv')";
            $rs = $this->conn;
            $result = $rs->query($sqlQuery);
            if($result==TRUE){
                return 1;
        ...
        //TODO: UPDATE the session as admin may be demoted to member, access privilege to dashboard
